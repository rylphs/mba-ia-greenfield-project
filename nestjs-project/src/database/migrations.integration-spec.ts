...
import { CreateVideos1790357447592 } from './migrations/1790357447592-CreateVideos';
...
// DROP TABLE ... CASCADE removes the columns but not the enum types those
// columns used — a leftover type collides with the next run's CREATE TYPE.
...
describe('Database migrations (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    await dataSource.query(
      `DROP TYPE IF EXISTS ${MANAGED_ENUM_TYPES.map((type) => `"${type}"`).join(', ')}`,
    );
  });
